...
public class NanoHTTPd {
	...
	private class HTTPSession implements Runnable
	{
		...
		public void run()
		{
			try
			{
				...
				Log.i("PAX", "NanoHTTPD uri: " + uri.toString());
			...
			boolean allowDirectoryListing )
	{
		...
		if ( f.isDirectory())
		{
			...
			else if ( allowDirectoryListing )
			{
				...
				for ( int i=0; i<files.length; ++i )
				{
					...
					// Show file size
					String extra = "";
					String nmt = (String) SybExt(files[i]);
					if ( curFile.isFile())
					{
						//extra = "target='_blank'";
						extra = "";
					...
					msg += "<a " + extra + " href=\"" + encodeUri( uri + files[i] ) + "\" " + nmt + ">" +
					files[i] + "</a>";
		...
		try
		{
			...
			// Support (simple) skipping:
				long startFrom = 0;
				//Johan Wieslander: changed to range instead of Range since headers previously changed to lowerCase
	 ...
	 */
	private static Hashtable theNMTTypes = new Hashtable();
	static
	{
		StringTokenizer st = new StringTokenizer(
				"mpg            vod "+
